...
class LogisticRegressor:
    ...
    def fit(
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            if i % print_every == 0 and verbose:
                print(f"Iteracion {i}: Loss {loss}")
            
            # Definimos la variable del error
            error = y_hat - y
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        # Usamos las funciones definidas antes de Ridge y Lasso puesto que elastic net cambina ambas
        lasso = self.lasso_regularization(np.zeros_like(dw), m, C)
        ridge = self.ridge_regularization(np.zeros_like(dw), m, C)
    ...
    def log_likelihood(y, y_hat):
        ...
        # Establecemos un corte para evitar pérdidas que sean infinitas
        eps = 1e-15
    ...
    def maximum_likelihood(y, y_hat):
        ...
        # Alias for log_likelihood as used in test
